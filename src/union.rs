    ...
    T: SortedSimdElement + Ord,
{
    ...
    while i < a.len() && j < b.len() {
        // Optimization: Use explicit if/else instead of match a[i].cmp(&b[j])
        // to avoid constructing the Ordering enum in hot loops, enabling better
    ...
    T::SimdVec: SimdPartialOrd<Mask = T::SimdMask>,
{
    ...
    while i < a.len() && j + lanes <= b.len() {
        ...
        for _ in 0..lanes {
            ...
            // Optimization: explicit if/else avoids Ordering enum overhead
            let val = if a[i] < b[j] {
                let v = a[i];
                i += 1;
                v
