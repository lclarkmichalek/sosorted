...
impl Bitmap {
    ...
    /// The input must be sorted in ascending order; unsorted inputs causing interleaved high-bit
    /// chunks will result in silent data loss as the internal `BTreeMap` overwrites existing
    /// containers, rather than strict memory-safety undefined behavior.
    ///
    /// # Examples
    ///
    /// ```
