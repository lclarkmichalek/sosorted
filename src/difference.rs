    ...
    T::SimdVec: SimdPartialOrd<Mask = T::SimdMask>,
{
    ...
    while i < a.len() && j + lanes <= b.len() {
        ...
        for _ in 0..lanes {
            ...
            // PERFORMANCE: Explicit if/else outperforms `match cmp` by avoiding enum construction
            if a[i] < b[j] {
    ...
    while i < a.len() && j < b.len() {
        // PERFORMANCE: Using explicit if/else instead of `match a[i].cmp(&b[j])`
        // avoids constructing the `std::cmp::Ordering` enum in hot loops,
        // which improves LLVM instruction pipelining and branch prediction.
        if a[i] < b[j] {
            count += 1;
            i += 1;
        } else if a[i] > b[j] {
