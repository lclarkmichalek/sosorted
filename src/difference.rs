    ...
    T::SimdVec: SimdPartialOrd<Mask = T::SimdMask>,
{
    ...
    while i < a.len() && j + lanes <= b.len() {
        ...
        for _ in 0..lanes {
            ...
            // OPTIMIZATION: Replaced match a[i].cmp(&b[j]) with explicit if/else
            // to avoid Ordering enum construction, improving performance in hot loops.
            if a[i] < b[j] {
    ...
    fn test_difference_size_fuzz() {
        ...
        for _ in 0..TEST_ITERATION_COUNT {
            ...
            while i < a_data.len() {
                ...
                // OPTIMIZATION: Replaced match a_data[i].cmp(&b_data[j]) with explicit if/else
                // to avoid Ordering enum construction, improving performance in hot loops.
                if a_data[i] < b_data[j] {
                    expected += 1;
                    i += 1;
                } else if b_data[j] < a_data[i] {
                    j += 1;
                } else {
