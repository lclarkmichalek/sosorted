    ...
    T::SimdVec: SimdPartialOrd<Mask = T::SimdMask>,
{
    ...
    while i < a.len() && j < b.len() {
        // OPTIMIZATION: Replacing match a.cmp(&b) with explicit if/else
        // avoids constructing the Ordering enum in this loop,
        // which helps performance for scalar fallback
        if a[i] < b[j] {
            count += 1;
            i += 1;
        } else if a[i] > b[j] {
    ...
    T::SimdVec: SimdPartialOrd<Mask = T::SimdMask>,
{
    ...
    while i < a.len() && j < b.len() {
        // OPTIMIZATION: Replacing match a.cmp(&b) with explicit if/else
        // avoids constructing the Ordering enum
        if a[i] < b[j] {
            dest[write] = a[i];
            write += 1;
            i += 1;
