...
su - runner -c 'source $HOME/.cargo/env && cd /tmp && cargo init --name warmup warmup_project && cd warmup_project && cargo build --release 2>/dev/null' || true
...
# 4. Set up ${timeout_seconds}-second (${timeout_seconds/60} minute) timeout watchdog
echo "Starting ${timeout_seconds/60}-minute timeout watchdog..."
(
  sleep ${timeout_seconds}
  echo "=== TIMEOUT: ${timeout_seconds/60} minutes elapsed, forcing shutdown ==="
...
# 5. Run the runner pinned to CPU 1 (leave CPU 0 for OS tasks)
...
# Kill the watchdog if runner completes normally
